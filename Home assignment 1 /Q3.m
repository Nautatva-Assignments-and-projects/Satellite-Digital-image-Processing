length1=length(HAImage(:,1));
newImage = zeros([length1 4]);

for j = 1:4
    for i = 1:length1
        newImage(i,j) = HAImage(i,j)*HAImage(i,j)*HAImage(i,j);
    end
end
minimum = 256; 
maximum = 0;
for j = 1:4   
    for i=1:length1
        if(newImage(i,j) < minimum)
            minimum = newImage(i,j);
        end
        if(newImage(i,j) > maximum)
    ...
    for i=1:length1
        newImage(i,j) = (255*(newImage(i,j)-minimum)/difference);
    end
end
clear i;
clear j;
...
%replacing HAImage with newImage
HAImage = newImage;
clear newImage;



% Now question 1 again
meansum = zeros([1 4]);
mean = zeros([1 4]);
median = zeros([1 4]);
mode = zeros([1 4]);
